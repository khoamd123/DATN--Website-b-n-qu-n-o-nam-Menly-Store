<?php
// Test action hủy sự kiện
echo "<!DOCTYPE html>";
echo "<html><head><title>Test Cancel Action</title>";
echo "<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css' rel='stylesheet'>";
echo "<link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css' rel='stylesheet'>";
echo "</head><body class='container mt-4'>";
echo "<h1>🧪 Test Cancel Action - Test hủy sự kiện</h1>";
...
try {
    ...
    if (!$testEvent) {
        ...
        $stmt->execute([
            ...
            'This is a test event for cancellation testing',
            'pending',
    ...
    echo "<h4>3. Test hủy sự kiện</h4>";
    
    $cancellationReason = "Test hủy sự kiện - " . date('Y-m-d H:i:s');
    ...
    $stmt = $pdo->prepare("UPDATE events SET " . implode(' = ?, ', array_keys($updateData)) . " = ? WHERE id = ?");
    $values = array_values($updateData);
    $values[] = $testEvent['id'];
    $result = $stmt->execute($values);
    
    if ($result) {
        echo "<p>✅ Đã hủy sự kiện thành công</p>";
        ...
        echo "<p>Cancellation Reason: " . ($updatedEvent['cancellation_reason'] ?? 'NULL') . "</p>";
        echo "<p>Cancelled At: " . ($updatedEvent['cancelled_at'] ?? 'NULL') . "</p>";
        ...
        echo "<p>❌ Không thể hủy sự kiện</p>";
    }
    echo "</div>";
    ...
    if ($updatedEvent['status'] === 'cancelled') {
        $reason = $updatedEvent['cancellation_reason'] ?? 'Sự kiện đã bị hủy bởi quản trị viên';
        ...
        echo "Hủy lúc: " . ($updatedEvent['cancelled_at'] ?? $updatedEvent['updated_at']);
        echo "</small>";
        echo "<small class='text-muted'>";
