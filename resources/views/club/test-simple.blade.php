...
<body>
            ...
            <div class="club-card">
                ...
                <p><strong>Vai trò của bạn:</strong>
                    @if($userRole)
                        ...
                        @else
                            <span class="role-badge badge-none">{{ $userRole }}</span>
                        @endif
                    @else
                ...
                <p><strong>Trưởng CLB:</strong> {{ $club->leader ? $club->leader->name : 'Chưa có' }}</p>
                <p><strong>Số thành viên:</strong> {{ $club->activeMembers->count() }}</p>

                <div class="test-buttons">
                    <button class="btn btn-leader" onclick="testPermission('leader', {{ $club->id }})">
                        🔑 Test Quyền Leader
                    </button>
                    <button class="btn btn-officer" onclick="testPermission('officer', {{ $club->id }})">
                        👔 Test Quyền Officer
                    </button>
                    <button class="btn btn-member" onclick="testPermission('member', {{ $club->id }})">
                        👤 Test Quyền Member
                    </button>
                    <button class="btn btn-info" onclick="getClubInfo({{ $club->id }})">
                        ℹ️ Thông tin CLB
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function testPermission(permissionType, clubId) {
            fetch(`{{ url('/club/test') }}/${permissionType}`, {
                method: 'POST',
                headers: {
