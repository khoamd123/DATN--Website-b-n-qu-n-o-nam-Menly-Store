    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.club-resources.update', $resource->id) }}" method="POST" enctype="multipart/form-data" onsubmit="debugFormData(event); return submitFormWithDeletions(event)">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="card-body">
                                                ...
                                                <div class="row" id="current-files-container">
                                                    @foreach($resource->files as $index => $file)
                                                        <div class="col-md-4 col-sm-6 mb-3">
                                                            <div class="card current-file-card {{ $file->is_primary ? 'border-primary' : '' }}" data-file-id="{{ $file->id }}">
                                                                <div class="card-body p-2">
                                                                    ...
                                                                    <div class="mt-2 text-center">
                                                                        <button type="button" class="btn btn-sm btn-danger" onclick="console.log('File delete button clicked for ID: {{ $file->id }}'); removeCurrentFile({{ $file->id }})">
                                                                            <i class="fas fa-trash"></i> Xóa
                                                                        </button>
                                                                        @if(!$file->is_primary)
                                                                            <button type="button" class="btn btn-sm btn-primary" onclick="console.log('File primary button clicked for ID: {{ $file->id }}'); setCurrentFilePrimary({{ $file->id }})">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="file" class="form-control @error('files') is-invalid @enderror" 
                                                   id="files" name="files[]" accept=".doc,.docx,.xls,.xlsx" multiple onchange="previewNewFiles(this)">
                                        ...
                                        <div class="form-group" id="file-preview-section" style="display: none;">
                                            ...
                                            <div class="file-preview-container">
                                                ...
                                                <div id="video-preview" style="display: none;">
                                                    <div class="card">
                                                        <div class="card-body">
                                                            <video id="preview-video" controls class="img-fluid rounded" style="max-height: 300px; width: 100%;">
                                        ...
                                        @if($resource->images && $resource->images->count() > 0)
                                            <div class="form-group">
                                                <label>Album hình ảnh hiện tại ({{ $resource->images->count() }} ảnh):</label>
                                                <div class="row" id="current-images-container">
                                                    @foreach($resource->images as $index => $image)
                                                        <div class="col-md-4 col-sm-6 mb-3">
                                                            <div class="card current-image-card {{ $image->is_primary ? 'border-primary' : '' }}" data-image-id="{{ $image->id }}">
                                                                <div class="card-body p-2">
                                                                    ...
                                                                    <div class="mt-2">
                                                                        <button type="button" class="btn btn-sm btn-danger" onclick="console.log('Image delete button clicked for ID: {{ $image->id }}'); removeCurrentImage({{ $image->id }})">
                                                                            <i class="fas fa-trash"></i> Xóa
                                                                        </button>
                                                                        @if(!$image->is_primary)
                                                                            <button type="button" class="btn btn-sm btn-primary" onclick="console.log('Image primary button clicked for ID: {{ $image->id }}'); setCurrentImagePrimary({{ $image->id }})">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <small class="form-text text-muted">
                                                Hỗ trợ: JPG, PNG, GIF (Tối đa 5MB mỗi ảnh, tối đa 10 ảnh)
                                            </small>
                                            <button type="button" class="btn btn-sm btn-info mt-2" onclick="testJavaScript()">
                                                Test JavaScript
                                            </button>
                                            <button type="button" class="btn btn-sm btn-warning mt-2" onclick="testDeleteFunction()">
...
<style>
.current-image-card, .new-image-card {
    transition: all 0.3s ease;
    border: 2px solid transparent;
}

.current-image-card:hover, .new-image-card:hover {
...
.current-image-card .card-body, .new-image-card .card-body {
    position: relative;
}

.current-image-card .btn, .new-image-card .btn {
    font-size: 0.75rem;
    padding: 0.25rem 0.5rem;
}

/* File Album Styles */
.current-file-card, .new-file-card {
    transition: all 0.3s ease;
    border: 2px solid transparent;
}

.current-file-card:hover, .new-file-card:hover {
    transform: translateY(-2px);
    box-shadow: 0 4px 8px rgba(0,0,0,0.1);
}

.current-file-card.border-primary, .new-file-card.border-primary {
    border-color: #007bff !important;
    box-shadow: 0 0 0 0.2rem rgba(0,123,255,.25);
}

.current-file-card .btn, .new-file-card .btn {
    font-size: 0.75rem;
    padding: 0.25rem 0.5rem;
}
