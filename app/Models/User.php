...
class User extends Authenticatable
{
     ...
     * Format: student@university.edu.vn -> 2024001
     */
    public static function generateStudentIdFromEmail($email)
    {
        // Extract domain and create student ID
        $domain = substr(strrchr($email, "@"), 1);
        
        // Check if it's a university email
        if (strpos($domain, 'edu.vn') !== false || strpos($domain, 'university') !== false) {
            // Generate student ID based on email hash
    ...
    {
        $domain = substr(strrchr($email, "@"), 1);
        return strpos($domain, 'edu.vn') !== false || 
               strpos($domain, 'university') !== false ||
               strpos($domain, 'student') !== false;
    }
